...
current="$(curl -fsSL 'https://api.wordpress.org/core/version-check/1.7/\?locale\=ja' | jq -r '.offers[0].current')"
sha1="$(curl -fsSL "https://ja.wordpress.org/wordpress-$current-ja.tar.gz.sha1")"
